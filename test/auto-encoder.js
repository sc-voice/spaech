(typeof describe === 'function') && describe("auto-encoder", function() {
  ...
  it("aggFrame() AN9.20_4.3", async()=>{
    ...
    let res = await AutoEncoder.frameSignal(signal, opts);
    let { frames } = res;
    let frameAgg = frames.map(f=>f[frameSize]);
    let data0Agg = frames.map(f=>f[0]);
    let iHalf = Math.floor(frameSize/2);
    let dataHalfAgg = frames.map(f=>f[iHalf]);
    let aggStats = Signal.stats(frameAgg);
    let data0Stats = Signal.stats(data0Agg);
    let dataHalfStats = Signal.stats(dataHalfAgg);
    verbose && console.log(`DEBUG`, {aggStats, data0Stats, dataHalfStats});
    let e = 0.35;
    let high = 1.00000000000001;

    // frame values are normalized to [-high,high] interval
    should(aggStats.max).above(1-e).below(high);
    should(data0Stats.max).above(1-e).below(high);
    should(dataHalfStats.max).above(1-e).below(high);
    should(aggStats.min).above(-high).below(-1+e);
    should(data0Stats.min).above(-high).below(-1+e);
    should(dataHalfStats.min).above(-high).below(-1+e);
  });
